...
def revenue_pre_save(sender, instance: Revenue, **kwargs):
    ...
    if not old_instance:
        ...
        converted_quantity = convert_or_fetch(currency_from, currency_to, real_quantity)
        ...
        update_or_create_annual_balance(
            converted_quantity, owner, new_instance.date.year, True
        )
    ...
    # Update action
        ...
        if (
            ...
            converted_old_quantity = convert_or_fetch(
                old_instance.currency_type, currency_to, old_instance.real_quantity
            )
            owner.balance += converted_quantity - converted_old_quantity
            owner.balance = round(owner.balance, 2)
            owner.save()
        ...
        # month and year needs to be checked
            ...
            # Create DateBalance or update it
            check_dates_and_update_date_balances(
                old_instance, converted_quantity, None, new_instance.date
            )
...
def revenue_pre_delete(sender, instance: Revenue, **kwargs):
    ...
    converted_quantity = convert_or_fetch(
        instance.currency_type, coin_to, instance.real_quantity
    )
    owner.balance -= converted_quantity
    owner.balance = round(owner.balance, 2)
    owner.save()
    # Create AnnualBalance or update it
    update_or_create_annual_balance(
        -converted_quantity, owner, instance.date.year, True
    )
    # Create MonthlyBalance or update it
    update_or_create_monthly_balance(
        -converted_quantity, owner, instance.date.year, instance.date.month, True
    )
